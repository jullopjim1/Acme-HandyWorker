...
public class FixUpTaskCustomerController extends AbstractController {
	...
	public ModelAndView show(final int fixUpTaskId, final RedirectAttributes redirectAttrs) {
		...
		try {
			...
			Assert.isTrue(fixUpTask.getCustomer().equals(this.customerService.findByUserAccount(LoginService.getPrincipal().getId())));
			...
			if (fixUpTask == null)
				redirectAttrs.addFlashAttribute("message1", "fixUpTask.error.unexist");
			else if (!(fixUpTask.getCustomer().equals(this.customerService.findByUserAccount(LoginService.getPrincipal().getId()))))
				redirectAttrs.addFlashAttribute("message1", "fixUpTask.error.notFromActor");
		}
		return result;
	}
