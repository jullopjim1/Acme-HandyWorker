...
public interface FinderRepository extends JpaRepository<Finder, Integer> {
	...
	@Query("select f from FixUpTask f where (f.ticker like %:keyword% or f.description like %:keyword% or f.adress like %:keyword%) and (f.deadline BETWEEN :dateMin and :dateMax) and (f.maxPrice BETWEEN :priceMin and :priceMax) and (f.category = (select c from Category c join c.name k where (key(k) like %:langCategory%) and (k like %:nameCategory%))) and (f.warranty.title like %:namewarranty%)")
	...
	@Query("select f from FixUpTask f where (f.ticker like %:keyword% or f.description like %:keyword% or f.adress like %:keyword%) and (f.deadline BETWEEN :dateMin and :dateMax) and (f.maxPrice BETWEEN :priceMin and :priceMax) and (f.warranty.title like %:namewarranty%)")
